...
def improductivos_report(request):
	...
	def color_bar_imp_total(promedio):
	...
	def grafica_produccion_total():
		...
		tope_improductivo = Producciones.objects.get(
			orden_produccion=op).tope_improductivo_produccion  # Query meta improductivo_produccion
		...
		bc.bars[0, 0].fillColor = HexColor(color_bar_imp_total(promedio_improductivo))
		# bc.strokeColor = colors.black # Dibuja un borde alrededor del grafico
		# bc.fillColor = colors.green # Fondo del grafico
	...
	def grafica_mantenimiento_total():
		...
		tope_improductivo = Producciones.objects.get(orden_produccion=op).tope_improductivo_mantenimiento
		promedio_improductivo = (improductivo_total * 100) / (tope_improductivo)
		# Coordenadas para empezar a dibujar el grafico
	...
	def get_improductivo(query_imp, hms):
		segundos = 0
		if hms:
			for x in query_imp:
